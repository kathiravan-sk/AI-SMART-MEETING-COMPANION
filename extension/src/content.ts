export {};
// Injected only after a user clicks Start captions. No passive host-wide collection.
const w = window as unknown as { __meetmindLoaded?: boolean };
if (!w.__meetmindLoaded) {
  ...
  const collect = () => {
    ...
    if (stamp - (seen.get(text) || 0) < 15000) return;
    seen.set(text, stamp);
    for (const [k, t] of seen) if (stamp - t > 60000) seen.delete(k);
