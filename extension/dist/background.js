...
(() => {
  ...
  async function api(path, body) {
    ...
    if (!r.ok) {
      ...
      throw new Error(
        typeof out.detail === "string" ? out.detail : `Request failed ${r.status}`
      );
    }
    return r.json();
  }
  async function sendChunk(capture, text) {
    const item = {
    ...
    try {
      ...
      await chrome.storage.local.set({
        ...
        captureError: "A caption could not be saved. Retry pending captions before stopping. " + e.message
      });
    }
  }
  async function stopCapture() {
    ...
    if (capture.mode === "captions") {
      await chrome.tabs.sendMessage(capture.tabId, { type: "STOP_CAPTIONS" }).catch(() => {
      });
  ...
  chrome.runtime.onMessage.addListener((message, sender, respond) => {
    ...
    if (message.type === "CAPTION") {
      queue = queue.then(async () => {
        const { capture } = await chrome.storage.local.get("capture");
        if (capture?.accepting && capture.mode === "captions" && sender.tab?.id === capture.tabId && typeof message.text === "string" && message.text.length <= 6e3)
    ...
    (async () => {
      ...
      if (message.type === "RETRY_PENDING") {
        ...
        await chrome.storage.local.set({
          unsent: failed,
          captureError: failed.length ? "Some captions are still pending. Resume the meeting and retry." : ""
        });
        return { ok: true };
      }
      if (message.type === "START_CAPTURE") {
        await stopCapture();
        const { apiUrl = "http://localhost:8000", token } = await chrome.storage.local.get(["apiUrl", "token"]);
        ...
        if (capture.mode === "captions") {
          ...
          if (!await chrome.offscreen.hasDocument())
            await chrome.offscreen.createDocument({
              ...
              justification: "Transcribe the meeting tab audio after the user explicitly starts capture."
            });
          try {
            const streamId = await new Promise(
              (resolve, reject) => chrome.tabCapture.getMediaStreamId(
                { targetTabId: tab.id },
                (id) => {
  ...
  chrome.tabs.onRemoved.addListener(async (tabId) => {
    ...
    if (capture?.tabId === tabId) {
      ...
      await chrome.storage.local.set({
        captureError: "The captured tab was closed. Your saved transcript is still available."
      });
    }
  });
})();
